...
- O DOCTYPE dos fragmentos fica de fora do produto, então só valem as cinco entidades do XML e as referências numéricas; `&nbsp;` e afins são recusados na verificação (Fase 5). Com ele saem também os valores padrão de atributos definidos na DTD, como o `@class` do DITA: no `product.xml`, o elemento só tem os atributos escritos no arquivo. Um deriver DITA no futuro terá de repor esses valores (ou ler a DTD) em vez de contar com eles.
